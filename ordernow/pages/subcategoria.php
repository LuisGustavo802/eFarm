<?php
    ...
    $pegar_dados_subcategoria = BD::conn()->prepare("SELECT titulo FROM `tabela_subcategorias` where slug = ?");
    $pegar_dados_subcategoria->execute(array($pegar_subcategoria));
    $fetch_sub = $pegar_dados_subcategoria->fetchObject();
    //Increment views
			...
			<div class="col-xl-9 col-lg-8 col-md-7">
				...
				<section class="lattest-product-area pb-40 category-list">
					<?php
							...
							$sql = "SELECT * FROM `tabela_produtos` where categoria = ? AND subcategoria = ? ORDER BY id DESC";
							$executar_cat = BD::conn()->prepare($sql);
							$executar_cat->execute(array($pegar_categoria, $pegar_subcategoria));
